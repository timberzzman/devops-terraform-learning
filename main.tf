...
# Exercice 06 : Automatic configuration
...
resource "scaleway_lb_certificate" "cert01" {
  ...
  name  = "webmdmfs1-mailly-cert"

  letsencrypt {
    common_name = "${scaleway_lb.base.ip_address}.sslip.io"
  }
}
...
resource "scaleway_lb_backend" "backend02" {
  lb_id            = scaleway_lb.base.id
  name             = "backend01"
  forward_protocol = "http"
  forward_port     = "443"
...
resource "scaleway_lb_frontend" "frontend02" {
  lb_id          = scaleway_lb.base.id
  backend_id     = scaleway_lb_backend.backend01.id
  name           = "frontend01"
  inbound_port   = "443"
  certificate_id = scaleway_lb_certificate.cert01.id
}
...
data "scaleway_instance_image" "web-server-image" {
  architecture = "x86_64"
  name         = "webmdm1-mailly-terraform"
}

resource "scaleway_instance_server" "web" {
  count              = 2
  type               = "DEV1-S"
  image              = data.scaleway_instance_image.web-server-image.id
